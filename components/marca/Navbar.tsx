  ...
  return (
    <>
          ...
          <nav className="hidden items-center gap-0.5 lg:flex xl:gap-1">
            {LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="whitespace-nowrap rounded-full px-2.5 py-2 text-[13px] font-medium text-neutral-600 transition-colors hover:bg-brand/10 hover:text-brand xl:px-3.5 xl:text-[13.5px]"
              >
                {link.label}
              </Link>
            ))}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="fixed inset-x-0 top-[76px] bottom-0 z-40 flex flex-col overflow-y-auto bg-white lg:hidden"
            initial={{ y: "-100%" }}
            animate={{ y: 0 }}
            exit={{ y: "-100%" }}
          ...
          >
            <nav className="flex flex-1 flex-col px-6 pt-2">
              {LINKS.map((link, i) => (
                ...
                >
                  <Link
                    ...
                    className={cn("flex items-center justify-between border-t border-neutral-100 py-4 text-lg font-medium text-neutral-900")}
                  >
                    {link.label}
              ...
              <motion.div
                ...
                className="mt-8 border-t border-neutral-100 pb-10 pt-8"
              >
                <Link
                  href="/login"
                  onClick={() => setOpen(false)}
                  className="block rounded-full bg-neutral-950 px-6 py-4 text-center text-sm font-medium text-white"
                >
                  Iniciar sesión
                </Link>
              </motion.div>
