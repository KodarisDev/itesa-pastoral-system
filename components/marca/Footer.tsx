...
const ACCESOS = [{ href: "/login", label: "Acceso de encargados" }];

export function Footer() {
  const year = new Date().getFullYear();

  return (
    <footer className="border-t border-neutral-100 bg-neutral-50 transition-colors duration-300">
      <div className="mx-auto grid max-w-[1440px] grid-cols-1 gap-10 px-4 py-16 sm:px-6 md:grid-cols-4 lg:px-8">
        ...
        <div>
          <h3 className="text-[11px] font-semibold uppercase tracking-widest text-neutral-400">Enlaces</h3>
          <ul className="mt-4 space-y-2.5">
            {ENLACES.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="text-sm text-neutral-600 transition-colors hover:text-brand">
                  {link.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          <h3 className="text-[11px] font-semibold uppercase tracking-widest text-neutral-400">Acceso</h3>
          <ul className="mt-4 space-y-2.5">
            {ACCESOS.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="text-sm text-neutral-600 transition-colors hover:text-brand">
      ...
      <div className="border-t border-neutral-100">
        <div className="mx-auto max-w-[1440px] px-4 py-6 text-center text-xs text-neutral-400 sm:px-6 lg:px-8">
