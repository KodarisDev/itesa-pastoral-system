      ...
      <StaggerContainer className="grid grid-cols-1 gap-6 sm:grid-cols-3">
        {NOTICIAS.map((n) => (
          <StaggerItem key={n.slug}>
            <article className="flex h-full flex-col overflow-hidden rounded-3xl border border-neutral-100">
              <Link
              ...
              <div className="flex flex-1 flex-col p-6">
                <div className="mb-3 flex items-center gap-2 text-[11px] font-medium uppercase tracking-widest text-brand">
                  ...
                  <span className="text-neutral-400">{n.fecha}</span>
                </div>
                ...
                <p className="mt-2 flex-1 text-sm leading-relaxed text-neutral-500">{n.resumen}</p>
                <Link
                  href={`/noticias/${n.slug}`}
