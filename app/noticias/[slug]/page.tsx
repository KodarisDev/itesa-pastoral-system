    ...
    <main id="contenido" className="bg-white text-neutral-950">
      ...
      <article className="px-4 pb-16 pt-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <nav aria-label="Ruta de navegación" className="mb-6 flex items-center gap-1.5 text-xs text-neutral-500">
          ...
          <div className="mb-3 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-widest text-brand">
            ...
            <span className="text-neutral-400">{noticia.fecha}</span>
          </div>
          <h1 className="text-balance text-[clamp(28px,4.5vw,44px)] font-bold leading-[1.1] tracking-tight text-neutral-950">
            {noticia.titulo}
          </h1>
      ...
      {relacionadas.length > 0 && (
        <section className="mx-auto max-w-[1440px] px-4 pb-24 pt-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-end justify-between gap-4 border-t border-neutral-100 pt-10">
            <div>
            ...
            <Link
              href="/#noticias"
              className="hidden shrink-0 items-center gap-1.5 rounded-full border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-600 transition-colors hover:border-brand hover:text-brand sm:inline-flex"
            >
              Ver todas
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            {relacionadas.map((n) => (
              <article key={n.slug} className="flex flex-col overflow-hidden rounded-3xl border border-neutral-100">
                <Link
                ...
                <div className="p-5">
                  ...
                  <p className="mt-1.5 line-clamp-2 text-sm text-neutral-500">{n.resumen}</p>
                  <Link
                    href={`/noticias/${n.slug}`}
